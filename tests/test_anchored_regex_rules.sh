...
{"id":"cg-test-price","anchor":"__cg_test_price__","expression":"{{anchor}}[[:space:]]*[:：][[:space:]]*[￥¥][[:space:]]*[0-9]{1,3}(?:,[0-9]{3})*(?:[.][0-9]{1,2})?","enabled":true}
EOF
...
regex_code="$(curl -sS -o "$OUTPUT_FILE" -w '%{http_code}' -X POST "$GATEWAY_URL/openai/responses" -H 'Authorization: Bearer dummy-token' -H 'Content-Type: application/json' -d '{"input":"__cg_test_price__: ￥ 100,000.00"}')"
[[ "$regex_code" == "403" ]]
grep -F '命中内容：__cg_test_price__: ￥ 100,000.00' "$OUTPUT_FILE" >/dev/null
...
invalid_code="$(curl -sS -o "$OUTPUT_FILE" -w '%{http_code}' -X POST "$GATEWAY_URL/regex-rules" -H 'X-API-Key: default-secret-token-please-change-me' -H 'Content-Type: application/json' -d '{"id":"invalid-backreference","anchor":"__cg_test_price__","expression":"{{anchor}}\\1"}')"
[[ "$invalid_code" == "400" ]]
